...
> 跨いで保たれ、`;` で確定する)。TIB 128 byte の制約上 **1 行 128 字
> 以内** に収める必要はありますが、**1 つの語の本体は何行でも続けられ
> ます**。
...
- **複数行 `:` 定義** が必須。`SAFE?` だけでも 128 byte の TIB に
  収まらない。各行ごとに ` ok` が出るのに compile mode が永続する
  挙動が標準的な Forth の REPL の姿。
