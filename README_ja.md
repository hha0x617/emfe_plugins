...
| プラグイン | ターゲット | 実装状況 |
|-----------|----------|----------|
| `mc6809` | Motorola 6809 (`em6809-core` Rust クレートを薄くラップ) | **公開済** — DLL ビルド + リリース同梱 |
| `mc68030` | Motorola 68030 | **公開済** — DLL ビルド + リリース同梱 |
| `z8000` | Zilog Z8000 ファミリ (Z8001/Z8002/Z8003/Z8004) | **部分対応 — Z8002 のみ** — DLL ビルド + リリース同梱。Z8002 (非セグメント, VM なし) は完全動作。Z8001 / Z8003 / Z8004 は Settings から選択可能だが現状 Z8002 挙動にフォールバック。セグメント・アドレッシングは Phase 2、VM / abort 対応は Phase 3 で対応予定。 |
| `em8` | 小規模な教育用 CPU | **公開済** — ABI 検証用、DLL ビルド + リリース同梱 |
| `rv32ima` | RISC-V RV32IMA | **設計ノートのみ** — `docs/` のみ存在、ソース未着手 |
| `api` | 共通 C ABI ヘッダ (`emfe_plugin.h`) | **ヘッダのみ** — プラグイン DLL ではなく、各プラグインが取り込む |

「公開済」のプラグインは push 毎に CI でビルドされ、タグ付きコミット
の GitHub Release に zip / インストーラとして同梱されます。`z8000`
DLL も毎リリースに同梱されますが、上記の Z8002 限定の制約付きです —
Settings で Z8001 / Z8003 / Z8004 を選んでも現状ではそれらの差分挙動を
発揮しません。「設計ノートのみ」「ヘッダのみ」の行は、ディレクトリ
構造に違和感を持たせないために明記しているもので、GitHub Release に
これらの DLL は **含まれません**。
